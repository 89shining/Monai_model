...
def build_model(backbone: str, ckpt_path: str, device: torch.device):
    model = DeepLab(num_classes=1, backbone=backbone, pretrained=False, downsample_factor=16).to(device)
...
def main():
    ...
    model = build_model(args.backbone, ckpt_path, device)
    ...
    for image_path in images:
        ...
        with torch.no_grad():
            for z in range(vol.shape[0]):
                ...
                sl = F.interpolate(sl, size=(512, 512), mode="bilinear", align_corners=False)
                sl = sl.repeat(1, 3, 1, 1).to(device)
                logits = model(sl)
