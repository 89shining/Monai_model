...
def dice_score_from_probs(probs: torch.Tensor, target: torch.Tensor, eps: float = 1e-6) -> float:
...
def build_model() -> nn.Module:
    return DDUNet2D(in_channels=1, out_channels=1)
...
def run_fold(args, fold: int, train_cases: List[CaseData], val_cases: List[CaseData], device: torch.device):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in train_loader:
            ...
            probs = model(x)
            loss = dice_bce_loss_from_probs(probs, y)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for x, y in val_loader:
                ...
                y = y.to(device)
                probs = model(x)
                val_loss += dice_bce_loss_from_probs(probs, y).item()
                val_dice += dice_score_from_probs(probs, y)
