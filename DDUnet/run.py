...
SAVE_ROOT = r"/home/wusi/Project_crop/Data/Eso_83/Networks/DDUNet/EsoCTV_All"
...
def collect_case_ids(data_root: str):
    imgs = sorted(glob.glob(os.path.join(data_root, 'imagesTr', '*_0000.nii.gz')))
    return sorted({image_case_id(p) for p in imgs})
...
def write_cv_results(runs_root: str, num_folds: int, seed: int, data_root: str):
    ids = collect_case_ids(data_root); kf = KFold(n_splits=num_folds, shuffle=True, random_state=seed)
    sizes = {fold: (len(tr), len(val)) for fold, (tr, val) in enumerate(kf.split(ids))}
    rows = []
    for fold in range(num_folds):
        fold_dir = os.path.join(runs_root, f'fold_{fold}')
        ckpt = os.path.join(fold_dir, f'best_model_fold{fold}.pth')
        be, bd = read_best_from_fold_epoch_csv(fold_dir); ntr, nval = sizes[fold]
        rows.append({'fold': fold, 'best_epoch': be, 'best_dice_fg': bd, 'num_train': ntr, 'num_val': nval, 'checkpoint': ckpt})
...
def pick_best_fold(cv_path: str) -> int:
    with open(cv_path, 'r', encoding='utf-8') as f: rows = list(csv.DictReader(f))
...
def main():
    ...
    train_py = os.path.join(os.path.dirname(__file__), 'train.py'); test_py = os.path.join(os.path.dirname(__file__), 'test.py')

    for fold in range(NUM_FOLDS):
        fold_dir = os.path.join(RUNS_ROOT, f'fold_{fold}')
        done_marker = os.path.join(fold_dir, 'fold_done.flag'); ckpt = os.path.join(fold_dir, f'best_model_fold{fold}.pth')
        if AUTO_RESUME and os.path.exists(done_marker) and os.path.exists(ckpt):
            continue
        cmd = [sys.executable, train_py, '--data_root', DATA_ROOT, '--runs_root', RUNS_ROOT, '--num_folds', str(NUM_FOLDS), '--seed', str(SEED), '--max_epochs', str(MAX_EPOCHS),
               '--early_stop_patience', str(EARLY_STOP_PATIENCE), '--batch_size', str(TRAIN_BATCH_SIZE), '--num_workers', str(NUM_WORKERS), '--lr', str(LR), '--eta_min', str(ETA_MIN),
               '--weight_decay', str(WEIGHT_DECAY), '--grad_clip', str(GRAD_CLIP), '--input_h', str(INPUT_H), '--input_w', str(INPUT_W), '--a_min', str(A_MIN), '--a_max', str(A_MAX), '--only_fold', str(fold)]
    ...
    with open(os.path.join(RUNS_ROOT, "selected_test_fold.txt"), "w", encoding="utf-8") as f:
        f.write(f"best_fold_by_val_dice={best_fold}\n")
    test_cmd = [sys.executable, test_py, '--data_root', DATA_ROOT, '--runs_root', RUNS_ROOT, '--save_dir', PRED_SAVE_DIR, '--batch_size', str(TEST_BATCH_SIZE),
                '--workers', str(NUM_WORKERS), '--input_h', str(INPUT_H), '--input_w', str(INPUT_W), '--threshold', str(THRESHOLD), '--fold', str(best_fold), '--a_min', str(A_MIN), '--a_max', str(A_MAX)]
