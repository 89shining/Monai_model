...
def dice_bce_loss_from_probs(pred_prob: torch.Tensor, target: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    target = target.float()
    p = pred_prob.contiguous().view(pred_prob.shape[0], -1)
    t = target.contiguous().view(target.shape[0], -1)
    inter = (p * t).sum(dim=1)
    den = p.sum(dim=1) + t.sum(dim=1)
    dice_loss = 1.0 - ((2.0 * inter + eps) / (den + eps))
    dice_loss = dice_loss.mean()
    bce_raw = F.binary_cross_entropy(pred_prob, target)
...
def get_transforms(args):
    ...
    if args.strong_aug:
        ...
        train_list.extend([
            RandAffined(keys=['image', 'label'], prob=0.1, rotate_range=(0.05, 0.05, 0.02), scale_range=(0.05, 0.05, 0.02),
                        mode=('bilinear', 'nearest'), padding_mode='border'),
...
def train_one_epoch(model, loader, optimizer, loss_func, device, args):
    ...
    for batch in loader:
        ...
        probs = model(images)
        loss = dice_bce_loss_from_probs(probs, labels)
        loss.backward()
        if args.grad_clip > 0:
...
def validate(model, loader, loss_func, dice_metric, post_pred, post_label, device, args):
    ...
    for batch in loader:
        ...
        val_loss += dice_bce_loss_from_probs(probs, labels).item()
