...
def main() -> None:
    parser = argparse.ArgumentParser(description="Standalone test for VNet with same logic as training script.")
    parser.add_argument("--fold_idx", type=int, default=0, help="Fold index used only for logging and default checkpoint name.")
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="",
        help="Path to checkpoint .pth. If empty, uses ../runs/VNet/fold_{fold_idx}/best_model_fold{fold_idx}.pth",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
